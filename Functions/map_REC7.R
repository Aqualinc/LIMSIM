#setwd(dir$data)
NZmap <- read.csv(paste(dir$data,"NZmap.csv",sep="/"))  # add coastline
...
MapRivers <- function(RECvar=REC.dat$usArea, MinOrder = 0, REC = REC.dat, pos="topleft", MyLegend = NULL,linevar=TRUE,   
                      MyRound = 1, myCol = "lightblue", myScale = 0.5, name="Variable", n.breaks=10, col.ord=F,
                      MyBreaks = NULL,MySeq = NULL,PlotTitle=NULL,p_leg=TRUE,is.cat=FALSE,props=TRUE,col.rnd=FALSE,col.lookup=FALSE,...) {
  ...
  if(is.null(MyLegend)) {
   ...
   # browser()
    ...
    RECvar.col <- rainbow(n.cats)
      ...
      #browser()
      RECvar.col<-as.character(col.lookup[match(cats,col.lookup[,1]),2])
    }
...
tnode<-REC_plot$tnode
  ...
  legend(x=pos, legend = pretty.cut.labs(divs, p=name, rounding = MyRound), col = RECvar.col,
          pch = c(15), bg = 'white', inset = c(0.01, 0.01), cex = 0.8)   
    }else{
    legend(pos, title=name, legend = cat.labs[match(sort(cat.labs),cat.labs)], col = RECvar.col[match(sort(cat.labs),cat.labs)],
           text.col = "black", pch = c(15), bg = 'white', inset = c(0.01, 0.01), cex = 0.8)    
    }
  }else{
